...
# This module supports asynchronous I/O on multiple file descriptors
  即该模块实现了linux网络编程里的IO多路复用技术，该模块源码列入Source Reading list
'''
import select
import sys
import os
...
# This module makes available standard errno system symbols.
  标准错误代码和错误提示。该模块有一些trick，因为在unix环境和windows环境下是不同的，所以
  该模块的错误代码并不是通用的，win下的只针对windows系统，unix下的只针对unix系统。
'''
import errno
'''
# python中关于多线程的操作可以使用thread和threading模块来实现，其中thread模块在Py3中已
  经改名为_thread，不再推荐使用。而threading模块是在thread之上进行了封装，也是推荐使用
  的多线程模块，本文主要基于threading模块进行介绍。在某些版本中thread模块可能不存在，
  要使用dump_threading来代替threading模块。
